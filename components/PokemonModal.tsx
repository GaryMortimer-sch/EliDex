...
import { generatePokemonSpeech } from '../services/geminiService';
import { decodeBase64, decodeAudioData, getAudioContext } from '../services/audioService';
...
const PokemonModal: React.FC<PokemonModalProps> = ({ pokemonId, onClose, autoPlay = false }) => {
  ...
  const handlePlayVoice = useCallback(async () => {
    ...
    try {
      ...
      const base64Audio = await generatePokemonSpeech(textToSpeak);
  ...
  // Handle AutoPlay when data is loaded
  useEffect(() => {
    if (autoPlay && !loading && pokemon && !hasAutoPlayed.current) {
      hasAutoPlayed.current = true;
      handlePlayVoice();
    }
  }, [autoPlay, loading, pokemon, handlePlayVoice]);
    ...
    return (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl p-8 max-w-lg w-full flex flex-col items-center shadow-2xl">
          ...
          <p className="mt-4 text-slate-600 font-medium">Summoning Data...</p>
        </div>
      </div>
    );
  }
